...
- **Model output:** [WRF idealized supercell (real run)](models/notebooks/wrf_idealized_supercell.ipynb) · [WRF post-proc](models/notebooks/wrf_postprocessing.ipynb) · [SWAN idealized (real run)](models/notebooks/swan_idealized_fetch.ipynb) · [ROMS upwelling (real run)](models/notebooks/roms_upwelling.ipynb) · [ROMS/CROCO post-proc](models/notebooks/roms_croco_output.ipynb) · [wave spectra](models/notebooks/wave_spectra.ipynb)
...
- [WRF Parameterization: Student-Friendly Guide](models/wrf-parameterization-guide.md) — Plain-language guide to all WRF physics options (microphysics, cumulus, PBL, surface layer, land surface, radiation) with simple explanations, when to use each scheme, Indonesia/tropical Maritime Continent recommendations, NCAR Tropical Suite, ready-to-use namelist configurations, decision flowchart, and common student mistakes.
...
- [CROCO: Complete Guide](models/croco-model-guide.md) — End-to-end CROCO v2.1 workflow: CROCO_PYTOOLS Python pre/post-processing (make_grid/forcing/clim/bry/tides), grid generation for Indonesian Seas, jobcomp compilation with CPP keys, croco.in configuration, AGRIF online 2-way nesting for Sunda/Lombok/Makassar straits, CROCO-NH non-hydrostatic mode, TPXO10/FES2022 tidal forcing, SLURM job submission, output diagnostics, and Indonesian waters (ITF transport, tidal constituents, BMKG operational context).
...
- [InaCAWO (BMKG Coupled System): Guide](models/inacawo-guide.md) — BMKG's operational Indonesia Coupled Atmosphere-Wave-Ocean system: COAWST-based architecture (WRF+ROMS+SWAN), 3 km domain covering all Indonesian waters (90°E–145°E), boundary condition download procedures (GFS/ECMWF, HYCOM/Mercator), build & run workflow, output products, and HPC specifications.
...
- [OpenIFS: Complete Guide](models/openifs-guide.md) — ECMWF's open-source IFS for research: installation (CMake/Docker), 3D NWP experiments at spectral resolutions (T21–TCo1279), Single-Column Model (SCM) with DYCOMS/BOMEX/TWPICE cases, static data and ERA5 initial conditions, IFS physics suite, and tropical/Indonesian applications.
- [SCHISM: Complete Guide](models/schism-model-guide.md) — End-to-end SCHISM v5.12 workflow: unstructured mesh generation (SMS/OceanMesh2D/pyschism), LSC² vertical coordinates, tidal forcing (bctides.in with TPXO10/FES2022), ERA5 sflux atmospheric forcing, GLORYS/HYCOM boundary conditions, param.nml configuration, wave coupling (WWM), sediment transport (SED3D), non-hydrostatic mode for internal waves, ParMETIS domain decomposition, pyschism pre/post-processing, and Indonesian waters application (ITF pathways, Lombok/Makassar/Sunda straits, BMKG operational setup).
